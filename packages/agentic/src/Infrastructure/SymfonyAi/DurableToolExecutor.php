...
final class DurableToolExecutor implements ToolExecutorInterface
{
    ...
    public function execute(array $toolCalls): \Generator
    {
        ...
        foreach ($toolCalls as $toolCall) {
            ...
            try {
                $result = $this->environment->await($this->stub->callTool($toolCall->getId(), $toolCall->getName(), $toolCall->getArguments()));
            } catch (DurableActivityFailedException $failure) {
                // Un outil en panne, tentatives épuisées, n'est pas une panne de la conversation : le
                // modèle le lit comme résultat et s'adapte. L'échec est au journal (`ActivityFailed`),
                // donc le rejeu reprend la même branche.
                yield new Progress('tool_failed', $failure->envelope()->message, $toolCall);
                $result = \sprintf('Échec de l\'outil « %s » : %s', $toolCall->getName(), $failure->envelope()->message);
            }
    ...
    /**
     * La frontière : le domaine ne parle pas le type du fournisseur.
     */
